...
def testlogin(request):
    if request.data.get('email') == "qotkdwns1324@naver.com" and request.data.get('password') == "1234":
        res = Response({"access_token": "thisisaccess"}, status=status.HTTP_200_OK)
        res.set_cookie("refresh_token", "thisisrefresh", httponly=True, secure=True, samesite="None")
        return res
    return Response(status=status.HTTP_404_NOT_FOUND)

@api_view(['GET'])
def testrefresh(request):
    if request.COOKIES.get("refresh_token") == "thisisrefresh":
        return Response({"access_token": "thisisnewaccess"}, status=status.HTTP_200_OK)
